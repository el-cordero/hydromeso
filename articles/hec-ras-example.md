...
GeoTIFFs. Inspect their units, dry-cell encoding, CRS, extent,
resolution, origin, and dimensions before classification. The following
self-contained example stands in for file-backed inputs:
...
it. Projecting coordinates is different. When geometry differs,
...
An AOI can be a `SpatVector` or supported path. By default both crop and
polygon mask are applied:
